...
call_initiate_model = ns.model('CallInitiate', {
...
class InitiateCall(Resource):
    ...
    def post(self):
        ...
        if not session_id or not call_type:
            logger.error(f"Données manquantes - session_id: {session_id}, call_type: {call_type}")
            return {"message": "session_id et call_type requis."}, 400
        ...
        if session.status == "completed":
            logger.warning(f"Tentative appel session terminée - session_id: {session_id}")
            return {"message": "Session terminée, appel impossible."}, 403

        try:
            message_type = f"{call_type}_call_signal"
            content = f"Appel {call_type} initié par {User.query.get(sender_id).username}"
            message = SessionMessage(
            ...
            try:
                ...
                logger.debug(f"Signal d'appel émis - session_id: {session_id}, message_id: {message.id}")
            except Exception as e:
                logger.error(f"Erreur notification WebSocket: {e}\n{traceback.format_exc()}")
                pass
            ...
            logger.error(f"Erreur initiation appel - session_id: {session_id}: {e}\n{traceback.format_exc()}")
            db.session.rollback()
            return {"message": f"Erreur serveur: {str(e)}"}, 500